...
if __name__ == "__main__":
	# argparse
	help_msg = "python EvtXHunt_python.py -f /dir/file"
	parser = argparse.ArgumentParser(description=help_msg, formatter_class=argparse.RawTextHelpFormatter)
	parser.add_argument('-f', '--file', type=str, required=True, help="Evtx Folder")
	...
	print("[+] Processing specified EVTX...")
				...
				for i in range(len(data)):
					...
					try:
						...
						if len(output) > 1:
							...
							print("[+] %-70s: %2s matches" % (data[i]['title'] , str(len(output))))
							analysis_dict['SIGMA Rule Name'] = data[i]['title']
							analysis_dict['SIGMA Rule Description'] = data[i]['description']
	...
	analysisDF = analysisDF.reindex(columns=['SIGMA Rule Name','SIGMA Rule Description','Count'])
