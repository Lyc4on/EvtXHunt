...
class EvtxIOCAnalysisIngestModuleFactory(IngestModuleFactoryAdapter):
    ...
    def createDataSourceIngestModule(self, ingestOptions):
        return EvtxIOCAnalysisIngestModule()
...
class EvtxIOCAnalysisIngestModule(DataSourceIngestModule):
    ...
    def process(self, dataSource, progressBar):
        ...
        while "logs.csv" not in os.listdir(temp_dir): self.log(Level.INFO, "Waiting for the csv files to be written")
        filelist = []
        ...
        if not filelist:
            message = IngestMessage.createMessage(IngestMessage.MessageType.DATA, "Evtx IOC Hunter", " No IOCs are found." )
        ...
        for path in filelist:
            ...
            for row in csv.DictReader(open(str(Result_Path))):             
                ...
                art.addAttributes(((BlackboardAttribute(attID_evt_rn, EvtxIOCAnalysisIngestModuleFactory.moduleName, str(row["SIGMA Rule"]))), \
                                    (BlackboardAttribute(attID_evt_rd, EvtxIOCAnalysisIngestModuleFactory.moduleName, str(row["SIGMA Description"]))), \
                                    (BlackboardAttribute(attID_evt_cn, EvtxIOCAnalysisIngestModuleFactory.moduleName, str(row["Computer"]))), \
                ...
                try: bboard.postArtifact(art, EvtxIOCAnalysisIngestModuleFactory.moduleName)
        ...
        #create and attach the artifact to the file
            ...
            #post the artifact to the blackboard to display 
            try: bboard.postArtifact(art2, EvtxIOCAnalysisIngestModuleFactory.moduleName)
            except Blackboard.BlackboardException as e: self.log(Level.SEVERE, "Error in posting artifact "+ art2.getDisplayName())  
       
        IngestServices.getInstance().postMessage(IngestMessage.createMessage(IngestMessage.MessageType.DATA, "Evtx IOC Hunter", " Evtx IOC Hunter execution completed successfully." ))
