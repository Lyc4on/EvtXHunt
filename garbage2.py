...
class EvtxIOCAnalysisIngestModule(DataSourceIngestModule):
    ...
    def process(self, dataSource, progressBar):
        ...
        targetFiles = list()
        path = "D:/SIT Year 2 Tri 1/2202 Digital Forensics/analysisCSV"
        self.log(Level.INFO, str(os.listdir(path)))
        
        for file in os.listdir(path):
            if(file.isdir():
               self.log(Level.INFO, "FOLDER")
            if file.endswith(".csv"):
                #self.log(Level.INFO, str(file)[:-4])
                targetFiles.append(str(file)[:-4])
                reader = csv.DictReader(open(path + "/" + file))
##                headers = reader.fieldnames
##                self.log(Level.INFO, headers[0])
                for row in reader:
                    self.log(Level.INFO, row)
                #for row in reader:
                #    self.log(Level.INFO, str(row))
        
        self.log(Level.INFO, "FILES FOUND " + str(len([name for name in os.listdir(path) if os.path.isfile(name)])))
        #Find the Windows Event Log Files
        files = []		
        fileManager = Case.getCurrentCase().getServices().getFileManager()
        # if self.List_Of_Events[0] == 'ALL':
        for i in targetFiles:
            files.append(fileManager.findFiles(dataSource, i))
            self.log(Level.INFO, "Adding to files var: " + i)
        files = fileManager.findFiles(dataSource, "trashed.csv")
        # else:
